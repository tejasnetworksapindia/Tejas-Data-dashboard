...
with st.sidebar:
    ...
    file_kpi = st.file_uploader("Upload KPI Report", type=['xlsx'])
    file_alarm = st.file_uploader("Upload Active Alarms", type=['xlsx'])
    file_fm = st.file_uploader("Upload FM Report", type=['xlsx'])
    file_vswr = st.file_uploader("Upload VSWR Report", type=['xlsx'])
    
    st.info("Note: Upload reports with 'Date' and 'Site Id' columns.")
...
if search_site and file_kpi:
    ...
    if not site_data.empty:
        ...
        # Band wise logic: '4G Cell Name' nundi Band extract cheyali (Ex: L1800, L2300)
        # Manam ikkada sample ga '4G Cell Name' lone band undi anukundham
        fig_traffic = px.bar(
            site_data, 
            x='4G Cell Name', 
        ...
        with col_kpi:
            ...
            st.dataframe(site_data[kpi_cols].sort_values(by='Date', ascending=False), use_container_width=True)

        with col_alarm:
            st.subheader("⚠️ HW Alarms & Faults")
            # Multiple files nundi alarms vethiki ikada chupinchali
            if file_alarm:
                df_al = pd.read_excel(file_alarm)
